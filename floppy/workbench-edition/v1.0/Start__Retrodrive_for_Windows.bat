...
:rungame
set "emulator=EMULATOR\WINDOWS\winuae32.exe"
set "config=EMULATOR\WINDOWS\config.uae"
cls
...
:: RUTA CORRECTA
set "gamefolder=GAMES%selected%"

:: COMANDO BASE CORRECTO
set "cmd="%emulator%" -f "%config%" -s use_gui=no"

set i=0

:: RECORRER DISCOS (CORRECTO)
for %%F in ("%gamefolder%*.adf") do (
set /a drive=!i!
call set "cmd=%%cmd%% -df!drive!=%%F"
set /a i+=1
)
echo Ejecutando:
